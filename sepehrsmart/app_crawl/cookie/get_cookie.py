...
class GetSepehrCaptcha:
    ...
    def get_driver(self, set_cookie=False):
        ...
        if set_cookie:
            ...
            # ===
            self.driver.set_page_load_timeout(15)  # Time in seconds
            # ====
            try:
                self.driver.get(f"https://{domain}")
            except:
                print("WARNING: Got an exception during runtime.")
    ...
    def get_validity(self,provider_code):
        try:
            ...
            kih_cookie = self.data['hotel'].get("KIH", {})
            cookies = kih_cookie.get("cookie", {})

            without_tour_providers = {
                "dayan": "KIH",
                "parmis": "KIH",
                "hamsafar": "KIH",
                "flamingo": "GSM",
                ...
                "yegane_fard": "GSM",
                "hamood": "GSM",
                "iman_amin_ziba_gsm": "GSM",
            }
            if provider_code in without_tour_providers.keys():
                main_destination = without_tour_providers[provider_code]
                kih_cookie = self.data['hotel'].get(main_destination, {})
                cookies = kih_cookie.get("cookie", {})
            ...
            headers = {
                ...
                'referer': f"https://{self.data['domain']}/Systems/FA/Reservation/Hotel_NewReservation_Search.aspx?action=display&rnd={rnd}",
            ...
            request(
                "POST",
                f"https://{self.data['domain']}/Systems/FA/Reservation/Hotel_NewReservation_Search.aspx",
                params=params,
                cookies=cookies,
                headers=headers,
                data=data,
            ...
            res = request(
                "GET",
                f"https://{self.data['domain']}/Systems/FA/Reservation/Hotel_NewReservation_Search.aspx?action=display&rnd={rnd}",
                cookies=cookies,
                verify=False
            )

            if "متاسفانه در اجرای دستورات خطایی رخ داده است" in res.text:
                print("This provider is OK:", provider_code)
                return True
            else:
                return False
        except Exception as e:
            print(f"An exception occurred during checking provider '{provider_code}': {e}")
            return False
        # ---
...
iman_amin_ziba_gsm_cookie=GetSepehrCaptcha(cookie_data.IMAN_AMIN_ZIBA_GSM)
