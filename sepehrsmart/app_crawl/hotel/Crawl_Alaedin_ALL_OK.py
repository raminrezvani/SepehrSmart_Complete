...
class Alaedin:
    ...
    def get_rooms(self,hotelCod, start_date, stay):
        ...
        response = requests.get(f"{base_url}{endpoint}", params=params, timeout=BUILD_TOUR_TIMEOUT)
        ...
        for i in range(len(json_data['room'])):
            try:
                ...
                if (json_data['room'][i]['isEntezar'] or
                        ...
                        json_data['room'][i]['isViewPriceBoard'] or
                        json_data['room'][i]['sumPrice'] == 0
                ):
    ...
    def get_result(self):
        try:
            ...
            rooms_executor = ThreadPoolExecutor(max_workers=min(len(parsed_hotels),50))
            ...
            try:
                ...
                completed, pending = wait(rooms_futures, timeout=BUILD_TOUR_TIMEOUT-1)
                ...
                # rooms_executor.shutdown(wait=False)
                pass
