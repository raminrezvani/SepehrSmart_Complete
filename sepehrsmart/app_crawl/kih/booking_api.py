...
class Booking:
    ...
    def get_result(self):
        try:
            ...
            try:
                response = requests.get(server_url, params=params, timeout=35)
                response.raise_for_status()  # Raise exception for bad status codes
                data = response.json()
                data = json.loads(data['text'])
            except requests.exceptions.Timeout:
                print("Booking ready tour -> The request to Booking service timed out!")
                return {'status': False, 'data': [], 'message': "اتمام زمان"}
            except (requests.RequestException, json.JSONDecodeError, KeyError, requests.HTTPError) as e:
                if response is not None:
                    print("Booking ready tour -> response.status_code:", response.status_code)
                    try:
                    ...
                    if result:
                        print("Booking ready tour -> response.json():", result)

                        # Check the inner status code from the JSON response
                        inner_status_code = result.get('status_code', None)

                        if inner_status_code == 504 or inner_status_code == 408:
                            print("Booking ready tour -> The distributed request timed out!")
                            return {'status': False, 'data': [], 'message': "اتمام زمان"}
                        
                        # You might want to handle the case where the inner status code is not 200
                        if inner_status_code != 200:
                            print(f"Booking ready tour -> Error: {result.get('message', 'Unknown error')}")
                            return {'status': False, 'data': [], 'message': "خطای نامشخص"}
                else:
                    # If response is None, likely due to a timeout or connection error.
                    print("Booking ready tour -> No response received (response is None likely due to a timeout or connection error).")
