...
class Alwin24:
    ...
    def get_with_selenium(self,use_cache):
        #==== Check cache ===
        redis_key = f"ready_{self.start_date}_{self.night_count}_{self.adults}_ALLWIN"
        if use_cache and has_key_cache(key=redis_key):
            result=get_cache(key=redis_key)
            print('Cache hitted')
            ...
            while (True):
                try:
                    driver.find_element(By.XPATH,
                                        '//*[@id="online-tour"]/div/div[2]/div[1]/div/div[1]/div[1]/div[1]/input').clear()
                    driver.find_element(By.XPATH,
                                        '//*[@id="online-tour"]/div/div[2]/div[1]/div/div[1]/div[1]/div[1]/input').send_keys(
                        'مشهد')
                    driver.find_element(By.XPATH,
                    ...
                    time.sleep(1)
                    driver.find_element(By.XPATH,
                                        '//*[@id="online-tour"]/div/div[2]/div[1]/div/div[1]/div[3]/div[1]/input').send_keys(
                        'کیش')
                    time.sleep(3)
