...
IS_RUNNING_IN_DOCKER=settings.IS_RUNNING_IN_DOCKER
...
def get_driver(**kwargs):
    ...
    # if IS_RUNNING_IN_DOCKER.lower() == "true":
    #     chrome_driver = "/usr/local/bin/chromedriver"
    # chrome_path = os.path.join(base_dir, chrome_driver)  # Ensure this path is correct
...
def get_driver_old(**kwargs):
    ...
    if IS_RUNNING_IN_DOCKER.lower() == "true":
        chrome_driver = "/usr/local/bin/chromedriver"
    chrome_path = os.path.join(base_dir, chrome_driver)
...
def add_dict_to_redis(key: str, data: [dict, list], expire_time) -> bool:
    ...
    """
    try:
        if '_Doing' not in key:
            key_doing=key+'_Doing'
            redis_connection.delete(key_doing)  # doing bayad hazf shavad!!!!
        else:
            print('')
