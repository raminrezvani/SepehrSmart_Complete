...
class BuildTour:
    def __init__(self, source, target, start_date, end_date, adults,priorityTimestamp, hotel_provider):
        self.source = source
        self.target = target
    ...
    def get_hotel(self,use_cache,iter):
        hotel = Hotel(source=self.source, target=self.target, start_date=self.start_date, end_date=self.end_date,
                      adults=self.adults,use_cache=use_cache, provider=self.hotel_provider, isAnalysiss=False,hotelstarAnalysis=[],priorityTimestamp=self.priorityTimestamp)
    ...
    def get_result(self, use_cache=True, iter=1):
        try:
            ...
            # just for debugging
            print("\n\n----------------------------")
            print("\t\t/sepehrsmart/app_crawl/build_tour/main.py line 50 -> list_process_threads():", list_process_threads())
            print("----------------------------\n\n")
            ...
            # just for debugging
            print("\n\n----------------------------")
            print("\t\t/sepehrsmart/app_crawl/build_tour/main.py line 76 -> list_process_threads():", list_process_threads())
            print("----------------------------\n\n")
            ...
            if self.hotel_provider and isinstance(self.hotel_provider, str) and self.hotel_provider != "":
                print("don't caching build tour results because it's just for a specific hotel provider")
            else:
            ...
            #     executor_cache.submit(add_cache, redis_key, result)
            # finally:
            #     executor_cache.shutdown(wait=False)
        ...
        # just for debugging
        print("\n\n----------------------------")
        print("\t\t/sepehrsmart/app_crawl/build_tour/main.py line 117 -> list_process_threads():", list_process_threads())
        print("----------------------------\n\n")
...
class BuildTourAnalysis():
    ...
    def get_analysis(self, start_date, end_date, range_number=7, use_cache=True, hotelstarAnalysis=[],priorityTimestamp=1):
        ...
        with ThreadPoolExecutor(max_workers=min(len(date_range), 10)) as executor:
            ...
            print(f' time BuildTour get_analysis --- {(datetime.now() - t1).total_seconds()}')
    ...
    def get_result(self, start_date, use_cache, iter, isAnalysiss=False, hotelstarAnalysis=[],priorityTimestamp=1):
        ...
        print(f' time BuildTour get_result --- {(datetime.now() - t1).total_seconds()}')
        # === Process Providers Efficiently ===
        t1=datetime.now()
        if results["hotel"]:
            results["providers"] = self.process_providers(results["hotel"])

        print(f' time BuildTour process_providers --- {(datetime.now() - t1).total_seconds()}')
