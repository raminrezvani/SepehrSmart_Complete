...
class Deltaban:
    isAnalysis=False
    def __init__(self, target, start_date, end_date, adults,isAnalysiss, isRefresh, hotelstarAnalysis=[],priorityTimestamp=1,
                 use_cache=True):
        self.target = target
    ...
    def get_hotels(self):
        ...
        while counter < 15:
            ...
            req = executeRequest(method="GET",url= req_url,
                                 ...
                                 use_cache=self.use_cache, forceGet=1)
            # req=req.json()
            req = json.loads(req)

            if req['status_code'] != 200:
                print(f"\nERROR: distributed request failed: {req['status_code']}, {req['text']}\n")
                continue
    ...
    def get_result(self):
        ...
        optimal_threads = min(len(hotels), 200) if len(hotels)!=0 else 1  # Set a reasonable max limit (e.g., 10)
        ...
        default_timeout = BUILD_TOUR_TIMEOUT
        if str(self.isRefresh).lower() == 'true':
            default_timeout = BUILD_TOUR_REFRESH_TIMEOUT            

        try:
            done, not_done = wait(futures, timeout=default_timeout)
            print("\n\n")
            print("number of completed tasks:", len(done))
            print("number of uncompleted tasks:", len(not_done))
            print("\n\n")
        except Exception as e:
            print(f"Error processing hotel rooms data: {e}")
        finally:
...
for tg in lst_targets1:
    if os.path.exists(f'Deltaban_hotels/Deltaban_hotels_info_{tg}.json'):
        ...
        ins = Deltaban(tg, start_date1, end_date1, adults, isAnalysiss, isRefresh, hotelstarAnalysis=[])
