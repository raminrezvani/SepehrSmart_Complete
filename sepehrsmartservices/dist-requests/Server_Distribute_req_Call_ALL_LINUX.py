...
# Define paths
script_name_without_extension = "Server_Distribute_req"
log_dir = os.path.join(os.getcwd(), "logs")  # Create logs directory in the current working directory
request_line_limit = 32768  # Set this to a value larger than the default (4094)
...
for port in ports:
    ...
    subprocess.Popen(
        # f"nohup python3 {script_name} {port} > {log_file} 2>&1 &",
        f"nohup gunicorn -w 3 --threads 4 --access-logfile - -b 0.0.0.0:{port} --limit-request-line {request_line_limit} --access-logformat '%(l)s %(u)s %(t)s \"%(r)s\" %(s)s %(b)s \"%(f)s\" \"%(a)s\" %(M)s' {script_name_without_extension}:app > {log_file} 2>&1 &",
        shell=True,
