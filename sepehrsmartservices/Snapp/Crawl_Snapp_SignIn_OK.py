...
def fetch_room_data(hotelID, date_from, date_to,priorityTimestamp,use_cache):
    ...
    print(f"Sleeping for {sleep_time:.2f} seconds to avoid sending concurrent requests to Snapp")
    ...
    for i in range(3):
        try:
            ...
            if not response['text'] or not json.loads(response['text']):
                print(f'response is NULL so retrying room data for hotel {hotelID}...')
                force=1
...
def get_hotel_rooms():
    ...
    executor_room = ThreadPoolExecutor(max_workers=200)  # Adjust the max_workers based on your needs
    ...
    completed, pending = wait(room_futures, timeout=BUILD_TOUR_TIMEOUT)
    ...
    try:
        ...
        for future in completed:
            ...
            try:
                ...
                print("\t\tjson_data_room:", json.dumps(json_data_room, indent=4, ensure_ascii=False))
                print(f"Error fetching room data for {htl['hotel_id']}: {e}")
    except Exception as e:
        print(f"Error in handling compeleted tasks: {e}")
    finally:
        executor_room.shutdown(wait=False)
